...
import logo from "./logo.png"; // upload logo Anda ke src dan beri nama logo.png

// Google Sheets config (dari Anda)
...
// --- small inline SVG icons (no external library needed) ---
const UsersIcon = () => (
  <svg
    viewBox="0 0 24 24"
    width="40"
...
export default function App() {
  ...
  return (
    <>
      {/* inline styles supaya Anda tidak perlu install Tailwind */}
      <style>{`
        :root { font-family: Inter, Roboto, Arial, sans-serif; }
        .app {
          min-height: 100vh;
          background: linear-gradient(180deg, #7f1d1d 0%, #ef4444 40%, #fff5f5 100%);
          display:flex;
        ...
        @media (max-width:640px) {
          .header { flex-direction:row; gap:12px; padding:12px; margin:16px 8px; }
      ...
      <div className="app">
        <header className="header" role="banner">
          {/** logo fallback: if image fails to load, show text */}
          <img
            src={logo}
            alt="Logo"
            className="logo"
          ...
          <div className="title-block">
            ...
            <div className="subtitle">
              Proyek Peningkatan Jalan Paket F di KIPP 1B
            </div>
          </div>
        </header>
        ...
        <main className="container" role="main">
          <div className="status-row">
            {loading ? (
              <div className="loading">Mengambil data...</div>
            ) : (
              <div></div>
            )}
          ...
          <section className="grid" aria-label="Kartu data HSE">
            <Card
              title="Tahun"
              value={data.tahun}
              Icon={CalendarIcon}
